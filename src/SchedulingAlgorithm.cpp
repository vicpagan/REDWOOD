...
*/
std::shared_ptr<SchedulingAlgorithm> SchedulingAlgorithm::create_scheduling_algorithm(const std::string& type, double delta_t, double delta_t_precision) {
    if (type == "dynamic") {
        return std::make_shared<SchedulingAlgorithmDynamic>(delta_t, delta_t_precision);
    }
    else {
...
std::string SchedulingAlgorithmDynamic::select_execution_option(
    ...
    for (const auto& [option_name, option_functions] : exec_options) {
        ...
        auto m_j = static_cast<long>(std::ceil(exec_option_time / probability_computation->get_delta_t()));
        auto n = static_cast<long>(std::ceil(remaining_time / probability_computation->get_delta_t()));
        auto probability_success = exp(-probability_computation->get_lambda() * m_j * probability_computation->get_delta_t());
