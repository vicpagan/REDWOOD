...
namespace wrench {
    ...
    std::vector<DeltaTResult> DeltaTEvaluator::evaluate(double min_delta_t, double max_delta_t, double step_size) {
        ...
        auto comparator_function = std::make_shared<ExpectedErrorComparator>(application_specs);
        auto algorithm = SchedulingAlgorithm::create_scheduling_algorithm("dynamic", application_specs, task_functions, probability_computation.get(), comparator_function.get());
